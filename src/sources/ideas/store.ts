...
// ============================================================================
// Critic Pipeline Functions
// ============================================================================

export async function getUnscoredIdeas(
  limit = 10,
  maxAgeDays = 7,
): Promise<readonly GeneratedIdea[]> {
  const db = getDb();
...
export async function getIdeasByRating(
  ...
  const db = getDb();
  if (opts?.category && opts?.stage) {
    return db`
      SELECT * FROM generated_ideas
      WHERE category = ${opts.category} AND pipeline_stage = ${opts.stage}
  ...
  if (opts?.category) {
    return db`
      SELECT * FROM generated_ideas
      WHERE category = ${opts.category} AND quality_score IS NOT NULL
      ORDER BY quality_score DESC, created_at DESC
      LIMIT ${limit}
    ` as Promise<GeneratedIdea[]>;
  }
  if (opts?.stage) {
    return db`
      SELECT * FROM generated_ideas
      WHERE pipeline_stage = ${opts.stage} AND quality_score IS NOT NULL
      ORDER BY quality_score DESC, created_at DESC
      LIMIT ${limit}
    ` as Promise<GeneratedIdea[]>;
  }
...
export async function getRatingInsights(): Promise<readonly RatingInsight[]> {
  ...
  return db`
    SELECT
      agent_id,
      COUNT(*)::int AS total,
      ROUND(AVG(quality_score)::numeric, 2)::float AS avg_score,
      COUNT(*) FILTER (WHERE pipeline_stage = 'validated')::int AS validated,
      COUNT(*) FILTER (WHERE pipeline_stage = 'archived')::int AS archived,
      ROUND(
        COUNT(*) FILTER (WHERE pipeline_stage = 'archived')::numeric
